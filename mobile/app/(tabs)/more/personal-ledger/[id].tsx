...
export default function PersonalLedgerScreen() {
  ...
  const { data: summary } = useQuery({
    queryKey: ['personal-ledger-summary', id],
    queryFn: () => customersApi.personalLedger.customerDetail(Number(id)),
    select: (res) => res.data,
    enabled: !!id,
  });

  const { data: entries, isLoading, refetch } = useQuery({
    queryKey: ['personal-ledger-entries', id, dateRange],
    queryFn: () => customersApi.personalLedger.entries.list({
      customer: Number(id),
  ...
  const addEntryMutation = useMutation({
    mutationFn: (data: any) => customersApi.personalLedger.entries.create({ ...data, customer: Number(id) }),
    onSuccess: () => {
      success('Entry added');
      setShowModal(false);
      setAmount('');
      setNotes('');
      queryClient.invalidateQueries({ queryKey: ['personal-ledger-summary', id] });
      queryClient.invalidateQueries({ queryKey: ['personal-ledger-entries', id] });
    },
    onError: (err: any) => showError(err.response?.data?.detail || 'Failed to add entry'),
  });

  const deleteEntryMutation = useMutation({
    mutationFn: (entryId: number) => customersApi.personalLedger.entries.delete(entryId),
    onSuccess: () => {
    ...
    <View style={styles.container}>
      ...
      <Portal>
        <Modal visible={showModal} onDismiss={() => setShowModal(false)} contentContainerStyle={styles.modal}>
          ...
          <TextInput label="Amount" mode="outlined" keyboardType="numeric" value={amount}
            onChangeText={setAmount} style={styles.input} />
          <TextInput label="Notes" mode="outlined" value={notes}
            onChangeText={setNotes} style={styles.input} />
          <Button mode="contained" onPress={handleAdd} loading={addEntryMutation.isPending}>Save</Button>
        </Modal>
      </Portal>
    </View>
  );
