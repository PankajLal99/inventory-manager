...
type BasketRow = LookupLine & {
  ...
  /** Per-line accepted credit; server requires &gt; 0 for every line (draft and instant); must not exceed sold unit price. */
  ...
  const submitCreate = () => {
      ...
      if (!Number.isFinite(acc) || acc <= 0) {
        showToast(
          mode === 'instant'
            ...
            : `Enter accepted price greater than 0 for each line (${row.product_name || 'item'}) — drafts require the same.`,
          'error',
        );
        return;
      }
    ...
    for (const row of basket) {
      ...
      if (raw !== '' && Number.isFinite(acc) && acc > 0 && acc > sold) {
        showToast(
          `Accepted price cannot exceed sold unit price (₹${formatNumber(sold)}) for ${row.product_name || 'item'}`,
          'error',
        );
        return;
      }
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">
                Store comes from the original sale for each scanned barcode. Choose a return tag (traffic lights) for every
                line. Draft (pending) and instant both require an accepted unit price greater than 0 per line — not above
                that line&apos;s sold unit price.
              </p>
            </div>
          </div>
        </div>
                ...
                {mode === 'instant' && (
                  <div className="mt-3 space-y-3">
                    ...
                    <p className="mt-2 text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                      Instant finalize writes one <span className="font-semibold">ledger credit</span> on the customer for
                      the return total (store credit balance goes <span className="font-semibold">up</span>). The entry
                          ...
                          <div className="flex-1 sm:w-32 min-w-[7rem]">
                            ...
                            <div className="relative">
                              ...
                              <Input
                                ...
                                min="0"
                                placeholder=""
                                title={`Required per line: greater than 0, not above sold ₹${formatNumber(unitSold)}`}
                            ...
                            {unitSold > 0 && (
                              ...
                              >
                                {acceptedTooHigh
                                  ? `Cannot exceed sold ₹${formatNumber(unitSold)}`
                                  : `≤ sold ₹${formatNumber(unitSold)} · required`}
                              </p>
                            )}
                          </div>
