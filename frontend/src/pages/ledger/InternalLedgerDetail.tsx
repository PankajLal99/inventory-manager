...
export default function InternalLedgerDetail() {
  ...
  const internalLedgerListPath = (() => {
    const query = searchParams.toString();
    return query ? `/internal-ledger?${query}` : '/internal-ledger';
  })();
  ...
  const { data: ledgerDetail, isLoading } = useQuery({
    queryKey: ['internal-ledger-customer-detail', customerId, filters.dateFrom, filters.dateTo, filters.entryType, filters.search],
    queryFn: () => {
      const params: any = {};
      ...
      return customersApi.internalLedger.customerDetail(parseInt(customerId || '0'), params);
    },
    enabled: !!customerId,
    retry: false,
  });
  ...
  const handleExportPDF = () => {
    ...
    const fileName = `internal_ledger_${customer?.name || 'customer'}_${format(new Date(), 'yyyy-MM-dd')}.pdf`;
    doc.save(fileName);
  };
  ...
  const handlePrint = () => {
    ...
    const printContent = `
      ...
      <html>
        <head>
          <title>${customer?.name || 'Customer'} Internal Ledger Statement</title>
          <style>
        ...
        <div className="flex items-center gap-4">
          <Button
            ...
            onClick={() => {
              ...
              navigate(internalLedgerListPath);
            }}
            size="sm"
          >
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              {customer?.name || 'Customer'} Shop Boys Ledger
            </h1>
            {customer?.phone && (
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No shop boys ledger entries for this customer</p>
            {hasActiveFilters && (
