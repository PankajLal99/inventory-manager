...
import { Users, Plus, Edit, Trash2, Search, Filter, DollarSign, ArrowLeft } from 'lucide-react';
import { toast } from '../../lib/toast';

export default function PersonalCustomers() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await customersApi.personalCustomers.list({ search });
  ...
  const { data: existingCustomersData } = useQuery({
    queryKey: ['personal-customers-search', debouncedNameSearch],
    queryFn: async () => {
      if (!debouncedNameSearch) return [];
      const response = await customersApi.personalCustomers.list({ search: debouncedNameSearch });
      return response.data;
    },
    enabled: !!debouncedNameSearch && showForm && !editingCustomer,
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (error: any) => {
      toast(error?.response?.data?.message || 'Failed to create personal customer', 'error');
    },
  });

  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: any }) => customersApi.personalCustomers.update(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['personal-customers'] });
      setShowForm(false);
      setEditingCustomer(null);
      resetForm();
      toast('Personal customer updated successfully', 'success');
    },
    onError: (error: any) => {
      toast(error?.response?.data?.message || 'Failed to update personal customer', 'error');
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: number) => customersApi.personalCustomers.delete(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['personal-customers'] });
      toast('Personal customer deleted successfully', 'success');
    },
    onError: (error: any) => {
      toast(error?.response?.data?.message || 'Failed to delete personal customer', 'error');
    },
  });
  ...
  const handleDelete = (id: number) => {
    if (confirm('Are you sure you want to delete this personal customer?')) {
  ...
  if (isLoading) {
    return <LoadingState message="Loading personal customers..." />;
  }

  if (error) {
    return (
      <ErrorState
        message="Error loading personal customers. Please try again."
        onRetry={() => queryClient.invalidateQueries({ queryKey: ['personal-customers'] })}
      />
    );
  }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Manage personal ledger customers"
        icon={Users}
        action={
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">Total Personal Customers</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">
      ...
      {filteredCustomers.length === 0 ? (
        <Card>
          <EmptyState
            icon={Users}
            title="No personal customers found"
            message="Get started by adding your first personal customer"
            action={
              <Button onClick={() => { resetForm(); setShowForm(true); }}>
                <Plus className="h-4 w-4 mr-2 inline" />
                Add Personal Customer
                ...
                <TableRow key={customer.id}>
                  ...
                  <TableCell align="right">
                    <div className="flex items-center justify-end gap-3">
                      <button
                        onClick={() => navigate(`/personal-ledger/${customer.id}`)}
      ...
      {showForm && (
        <Modal
          ...
          title={editingCustomer ? 'Edit Personal Customer' : 'Add Personal Customer'}
          size="lg"
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              {!editingCustomer && existingCustomers.length > 0 && (
                <div className="mt-2 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                  <p className="text-xs font-semibold text-yellow-800 mb-2">Similar existing personal customers found:</p>
                  ...
                  {existingCustomers.some((c: any) => c.name.toLowerCase() === formData.name.toLowerCase().trim()) && (
                    <p className="text-xs font-bold text-red-600 mt-2">
                      Warning: A personal customer with this exact name already exists.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
              ...
              >
                {(createMutation.isPending || updateMutation.isPending) ? 'Saving...' : 'Save Personal Customer'}
              </Button>
            </div>
          </form>
        </Modal>
