...
function HorizBar({ data, colors }: { data: BarItem[]; colors: string[] }) {
  const d = data.map(item => ({ ...item, name: item.name.length > 18 ? item.name.slice(0, 16) + '…' : item.name }));
  return (
    <ResponsiveContainer width="100%" height={Math.max(160, d.length * 36)}>
      <BarChart data={d} layout="vertical" margin={{ top: 0, right: 24, left: 0, bottom: 0 }}>
        <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#f0f0f0" />
        <XAxis type="number" tickFormatter={fmt} tick={{ fontSize: 11 }} />
        <YAxis type="category" dataKey="name" tick={{ fontSize: 12 }} width={120} />
        <Tooltip
          formatter={(value) => [`₹${formatNumber(value as number | string | undefined | null)}`, 'Revenue']}
          cursor={{ fill: '#f8fafc' }}
        />
...
export default function CategoryBrandChart({ categories, brands }: Props) {
  return (
