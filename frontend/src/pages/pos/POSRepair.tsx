...
export default function POS() {
  ...
  useEffect(() => {
    if (isInitialMount.current) {
      ...
      if (lastStoreIdRef.current === null) {
            ...
            <tbody>
              ${invoice.items && Array.isArray(invoice.items) ? invoice.items.map((item: any) => `
                <tr>
                  ...
                  <td class="col-price text-right">${formatCurrency(item.manual_unit_price || item.unit_price || '0')}</td>
                  <td class="col-total text-right">${formatCurrency(item.line_total || '0')}</td>
                </tr>
            ...
            <div className="w-full sm:w-auto">
              <div className="relative group">
                <div className="flex items-center gap-2 sm:gap-3 bg-white border-2 border-blue-200 rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 shadow-sm hover:shadow-md hover:border-blue-400 transition-all duration-200 cursor-pointer">
                  <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-sm sm:text-base font-semibold text-gray-900 truncate block uppercase">
                ...
                <select
                  ...
                  className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10 appearance-none uppercase"
                >
                  {filteredStores.map((store: any) => (
                    <option key={store.id} value={store.id.toString()}>
                      {store.name.toUpperCase()} {store.shop_type ? `(${store.shop_type.toUpperCase()})` : ''}
                    </option>
                  ))}
                </select>
              </div>
